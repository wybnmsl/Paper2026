...
- solve_instance：旧接口（MDL/MCL 阶段仍在使用），内部转调 solve_instance_with_spec
  + default_gls_spec，保证骨架一致；
- solve_instance_with_spec：在“同一 GLS 骨架”上，根据 GLSSpec 真正生效 init / candset /
  schedule / perturb / guidance / stopping / engine 等配置。

注意：
- 这里不依赖 LKH 风格的 gls_lk，实现上仅支持 engine.type="ls_basic"；
  其它取值会退回到 ls_basic。
"""
...
def _compute_tldr_traj_from_events(
    ...
    We approximate ell(t)=log(V_best(t)) as piecewise-constant between events, where:
    ...
    Returns (tLDR_traj or None, aux dict).
    """
    aux: Dict[str, Any] = {}
...
def _preheat_numba_once():
    """
    在当前进程内做一次极小规模预热，触发 numba JIT 编译，
    降低后续评估的冷启动时间。
    """
...
def _build_candidate_indices(dis_matrix: np.ndarray, k: int, use_mst: bool = True) -> np.ndarray:
    ...
    构造候选集：
      - 先做标准 kNN；
      - 若 use_mst=True，则把 MST 边并入候选集；
      - 最终返回 shape=(n, K_max) 的 int64 数组，每行长度一致。
    """
...
def _get_candidate_indices(dis_matrix: np.ndarray, k: int, use_mst: bool = True) -> np.ndarray:
    """
    带缓存的候选集构造。
    cache key 中包含 id(dis_matrix) 和规模，避免跨实例串用。
    """
...
def solve_instance(n, opt_cost, dis_matrix, coord, time_limit, ite_max, perturbation_moves, heuristic):
    """
    旧接口：保留不变（MDL/MCL 阶段依然使用）。
    内部统一走 solve_instance_with_spec + default_gls_spec，保证骨架一致。
    """
...
def solve_instance_with_spec(n, opt_cost, dis_matrix, coord,
    ...
    # trajectory-aware tLDR(T) from incumbent events (paper definition)
